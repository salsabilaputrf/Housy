...
func (r *repository) MultiParameter(params url.Values) ([]models.Property, error) {
	...
	err := r.db.Where("type_rent = ? AND price < ? AND bedroom = ? AND bathroom = ? AND amenities = ? AND created_at = ?", typeRent, price, bedroom, bathroom, amenities, date).Preload("City").Find(&properties).Error
