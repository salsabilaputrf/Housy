...
func (r *repository) MultiParameter(params url.Values) ([]models.Property, error) {
	...
	err := r.db.Where("type_rent = ? OR price < ? OR bedroom = ? OR bathroom = ? OR amenities = ? OR created_at ILIKE ?", typeRent, price, bedroom, bathroom, amenities, "%"+date+"%").Preload("City").Find(&properties).Error
